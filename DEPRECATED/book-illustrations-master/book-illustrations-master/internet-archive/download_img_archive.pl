...
while($buf=~/<a href="([^"]+)"/g) {
    ...
    if ($url=~/_tif\.zip/) {$tiffurl = $url; $EXT = 'tif'; } #disable TIFF images for now as they tend to be lower quality...
...
# Tiny change, since I am putting the kdu utility in a directory called "bin"
	    $CMDSBUF_PROCESS .= "./bin/kdu_buffered_expand -i $CACHEDIR/$INFILE/images/${IMAGEINFILE}_${rawfilepagenum}.$EXT -o $CACHEDIR/$INFILE/images/KDUCONVERT.ppm -num_threads 0 -int_region \"{$t,$l},{$height,$width}\" > /dev/null 2>&1\n";
